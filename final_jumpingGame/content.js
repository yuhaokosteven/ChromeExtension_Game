...
chrome.runtime.onMessage.addListener(function(message) {
  ...
  function fireBullet(type) {
    ...
    const dist = 30; //radial distance mech head
  ...
  function spawn() { //spawns bodies and map elements
    ...
    for (let i = 0; i < 200; i++) { //random bouncy circles
      // mapRect(200 + (Math.random()) * canvas.width, 0 + (Math.random()) * canvas.height, 0 + (Math.random() + 1) * 100, 20,
      mapRect(200 + (Math.random()) * 10000, 0 + (Math.random()) * 1000, 0 + (Math.random() + 1) * 100, 20, {
        restitution: 0.8,
      });
    }

    for (let j = 0; j < 3; j++) {
      for (let i = 0; i < 5; i++) { //a stack of boxes
        body[body.length] = Bodies.rectangle(200 + j * 30, 0 + i * 21, 25, 21);
      }
    ...
    mapRect(0, 150, 300, 20); //left platform

    for (let i = 0; i < 100; i++) { //random bouncy circles
      // mapRect(200 + (Math.random()) * canvas.width, 0 + (Math.random()) * canvas.height, 0 + (Math.random() + 1) * 100, 20,
      mapRect(200 + (Math.random()) * 5000, 0 + (Math.random()) * 3000, 0 + (Math.random() + 1) * 100, 20, {
        restitution: 0.8,
      });
    }


    mapRect(0, 1900, canvas.width, 20); //down platform
