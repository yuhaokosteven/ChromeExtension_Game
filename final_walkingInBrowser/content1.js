...
chrome.runtime.onMessage.addListener(function(message) {
  ...
  //   elementLoc.push(e[i].getBoundingClientRect());
  // }
  ...
  const gameProto = function() {
    ...
    this.gravityFlip = function() {
      if (keys[82] && this.buttonCD < this.cycle) {
        ...
        for (let i = 0; i < mapStatic.length; i++) {
          Matter.Body.setPosition(mapStatic[i], {
            x: mapStaitc[i].position.x,
            y: -mapStatic[i].position.y
          })
          //Matter.Body.rotate(map[i], Math.PI)
        }
  ...
  const mechProto = function() {
    ...
    this.keyMove = function() {
      if (this.onGround) { //on ground **********************
        if (this.crouch) { //crouch
          ...
          this.doCrouch();
          player.frictionAir = 0.5;
        } else if ((keys[38] || keys[87]) && this.buttonCD_jump + 20 < game.cycle) { //jump
        ...
        if (this.buttonCD_jump + 60 > game.cycle && //just pressed jump
          !(keys[38] || keys[87]) && //but not pressing jump key
    ...
    this.deathCheck = function() {
      if (this.y > 5000) { // if player is 4000px deep reset to spawn Position and Velocity
  ...
  function fireBullet(type) {
    ...
    var bulletX = mech.x + dist * Math.cos(mech.angle)
    var bulletY = mech.y + dist * Math.sin(mech.angle)
    bullet[len] = Bodies.rectangle(bulletX, bulletY, 10, 5, {
      angle: dir,
      //density: 0.001,
      //friction: 0.05,
      frictionAir: 0,
    ...
    // var a = document.getElementById('newsletter-submit')
    // if (790 < bulletX < 1129) {
  ...
  function bulletLoop() {
    ...
    if (keys[77] && fireBulletCD < game.cycle && !game.isPaused) {
  ...
  function spawn() { //spawns bodies and map elements
    function BodyRect(x, y, width, height, properties) { //speeds up adding reactangles to map array
      body[body.length] = Bodies.rectangle(x, y, width, height, properties);
    }
    ...
    // var elementLocSpan = [];
    // for (var i = 0; i < e.length; i++) {
    //   elementLocSpan.push(e[i].getBoundingClientRect());
    // }
    // for (let i = 0; i < elementLocSpan.length; i++) { //a stack of boxes
    //   body[body.length] = Bodies.rectangle(elementLocSpan[i].x, elementLocSpan[i].y, elementLocSpan[i].width, elementLocSpan[i].height, {
    //     restitution: 0.5
    //   });
    // }
    ...
    function mapRectStatic(x, y, width, height, properties) { //addes reactangles to map array
      mapStatic[mapStatic.length] = Bodies.rectangle(x + width / 2, y + height / 2, width, height, properties);
    }
    ...
    // for (let i = 0; i < elementLoc.length; i++) { //random bouncy circles
    //   mapRect(elementLoc[i].x, elementLoc[i].y, elementLoc[i].width, elementLoc[i].height, {
    //     restitution: 0.8,
    //   });
    // }

    // mapRectS(0, 1900, canvas.width, 20); //down platform
    // mapRectStatic(0, 150, 200, 20); //left platform

    // mapRectStatic(-20, 0, 20, canvas.height); //left platform
    // mapRectStatic(canvas.width, 0, 20, canvas.height); //left platform
    // mapRectStatic(0, canvas.height, canvas.width, 20); //left platform
    ...
    var up = 0;
    for (var i = 0; i < elementLoc.length; i++) {
      mapRect(elementLoc[i].x, elementLoc[i].y, elementLoc[i].width, elementLoc[i].height, {
        restitution: 0.8,
      });
      // up += 1;
    ...
    var elementLocA = [];
    for (var i = 0; i < e.length; i++) {
      elementLocA.push(e[i].getBoundingClientRect());
    }

    var up = 0;
    for (var i = 10; i < elementLocA.length; i++) {
      mapRect(elementLocA[i].x, elementLocA[i].y, elementLocA[i].width, elementLocA[i].height, {
        restitution: 0.8,
      });
      // up += 1;
    }
    ...
    //
    //   for (var i = 0; i < elementLoc.length; i++) {
    //     mapRect(elementLoc[i].x, elementLoc[i].y + up, elementLoc[i].width, elementLoc[i].height, {
    //       restitution: 0.8,
    //     });
    //     // up += 1;
    //   }
    ...
    // for (let i = 0; i < 15; i++) { //random bouncy circles
    //   // mapRect(200 + (Math.random()) * canvas.width, 0 + (Math.random()) * canvas.height, 0 + (Math.random() + 1) * 100, 20,
    //   mapRectStatic((Math.random()) * canvas.width, canvas.height - ((Math.abs(0.5 - Math.random()))) / 2 * canvas.height, 50, 20, {
    //     restitution: 0.8,
    //   });
    // }
    ...
    // map[map.length] = Bodies.rectangle(0, canvas.height, canvas.width, 200); //ground
    ...
    for (let i = 0; i < mapStatic.length; i++) {
      mapStatic[i].collisionFilter.group = -1;
      Matter.Body.setStatic(mapStatic[i], true); //make static
      World.add(engine.world, mapStatic[i]); //add to world
    }
    for (let i = 0; i < cons.length; i++) {
  ...
  function drawMap() {
    ...
    ctx.beginPath();
    for (let i = 0; i < mapStatic.length; i += 1) {
      let vertices = mapStatic[i].vertices;
      ctx.moveTo(vertices[0].x, vertices[0].y);
      for (let j = 1; j < vertices.length; j += 1) {
        ctx.lineTo(vertices[j].x, vertices[j].y);
      }
    ...
    // ctx.fillStyle = 'rgba(0,0,0,0)';
    ctx.fillStyle = '#444'; //static color
