  ...
  */
  ...
  const mechProto = function() {
    ...
    this.look = function() {
      ...
      //   mX = canvas.width * 0.8;
      // } else if (mX < canvas.width * 0.1) {
      //   mX = canvas.width * 0.1;
      // }
      // let mY = this.mouse.y;
      // if (mY > canvas.height * 0.8) {
      //   mY = canvas.height * 0.8;
      // } else if (mY < canvas.height * 0.1) {
      //   mY = canvas.height * 0.1;
      // }
      // //set mouse look
    ...
    this.deathCheck = function() {
      if (this.y > 7000) { // if player is 4000px deep reset to spawn Position and Velocity
    ...
    this.draw = function() {
      ...
      grd.addColorStop(0, "#0071b6");
      grd.addColorStop(1, "#019fff");
      ctx.fillStyle = grd;
      ctx.beginPath();
      ctx.ellipse(-30, 0, 10, 10, 180 * Math.PI / 180, 0, 2 * Math.PI);
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      // ctx.moveTo(0, 0);
      ...
      ctx.ellipse(0, 0, 30, 30, 0 * Math.PI / 180, 0, 2 * Math.PI);
      ctx.ellipse(15, 0, 4, 4, 0, 0, 2 * Math.PI);
      ctx.ellipse(15, 0, 15, 24, 180 * Math.PI / 180, 0, 2 * Math.PI);
  ...
  function fireBullet(type) {
    ...
    bullet[len] = Bodies.rectangle(mech.x + dist * Math.cos(mech.angle), mech.y + dist * Math.sin(mech.angle), 10, 5, {
      angle: dir,
      //density: 0.001,
      //friction: 0.05,
      frictionAir: 0,
  ...
  function bulletLoop() {
    ...
    if (keys[66] && fireBulletCD < game.cycle && !game.isPaused) {
  ...
  function spawn() { //spawns bodies and map elements
    ...
    // for (var i = 100; i < elementLocA.length; i += 50) {
    for (let j = 10; j < elementLocA.length; j += 30) { //a stack of boxes
      body[body.length] = Bodies.circle(elementLocA[j].x + elementLocA[j].width / 2, elementLocA[j].y - 40, 20, 20);
    }
    // }
    // for (let i = 0; i < 10; i++) { //random bouncy circles
    //   body[body.length] = Bodies.circle(elementLocA[Math.random() * 10].x, elementLocA[Math.random() * 10].y, 7 + Math.ceil(Math.random() * 30), {
    //     restitution: 0.8,
    //   })
    // }
  ...
  function cycle() {
    ...
    game.wipe();
    ctx.save();
    ctx.translate(0, -mech.y + 50);
    window.scrollTo(0, mech.y - 50);
    drawCons();
    drawBody();
    mech.draw();
    drawMap();
